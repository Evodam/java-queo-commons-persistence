...
/**
 * Just a copy of {@link BeanValidationIntegrator} that is modified to load a {@link BeanValidation20TypeSafeActivator}
 * instead of a {@link TypeSafeActivator}.
 * 
 * @author Steve Ebersole
 * @author Ralph Engelmann
 */
public class BeanValidation20Integrator implements Integrator {
    private static final CoreMessageLogger LOG = Logger.getMessageLogger(CoreMessageLogger.class,
            BeanValidation20Integrator.class.getName());
    ...
    public static void validateFactory(Object object) {
        try {
            ...
            final Class activatorClass = BeanValidationIntegrator.class.getClassLoader()
                    .loadClass(ACTIVATOR_CLASS_NAME);
            try {
                final Method validateMethod = activatorClass.getMethod(VALIDATE_SUPPLIED_FACTORY_METHOD_NAME,
                        Object.class);
            ...
            throw new HibernateException("Could not locate BeanValidation20TypeSafeActivator class", e);
        }
    }
    ...
    public void integrate(final Metadata metadata, final SessionFactoryImplementor sessionFactory,
        ...
        final Set<ValidationMode> modes = ValidationMode.getModes(cfgService.getSettings().get(MODE_20_PROPERTY));
        if (modes.size() > 1) {
        ...
        if (isBeanValidationApiAvailable(classLoaderService)) {
            ...
            try {
                ...
                @SuppressWarnings("unchecked")
                final Method activateMethod = typeSafeActivatorClass.getMethod(ACTIVATE_METHOD_NAME,
                ...
                try {
                    ...
                    throw new IntegrationException("Error activating Bean Validation integration",
    ...
    private void validateMissingBeanValidationApi(Set<ValidationMode> modes) {
        if (modes.contains(ValidationMode.CALLBACK)) {
            throw new IntegrationException(
                    "Bean Validation API was not available, but 'callback' validation was requested");
        }
